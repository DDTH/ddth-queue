...
public class LessLockingUniversalSingleStorageMySQLQueue
    ...
    public LessLockingUniversalSingleStorageMySQLQueue init() throws Exception {
         ...
         */
        SQL_UPDATE_EPHEMERAL_ID_TAKE = "UPDATE {0} SET {1}=?" + " WHERE " + WHERE_QUEUE_NAME_AND
                + "{1}=0" + (getFifo() ? (" ORDER BY " + COL_QUEUE_ID) : "") + " LIMIT 1";
        ...
        SQL_REMOVE_FROM_EPHEMERAL = "DELETE FROM {0}" + " WHERE " + WHERE_QUEUE_NAME_AND
                + COL_QUEUE_ID + "=?";
