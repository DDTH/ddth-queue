...
/*
 * mvn test -DskipTests=false -Dtest=com.github.ddth.queue.test.universal.idstr.mysql.TestMySQLLLQueueLong -DenableTestsMySql=true
 */

public class TestMySQLLLQueueLong extends BaseQueueLongTest<Long> {
    public TestMySQLLLQueueLong(String testName) {
        super(testName);
    }

    public static Test suite() {
        return new TestSuite(TestMySQLLLQueueLong.class);
    }
    ...
    protected IQueue<Long, byte[]> initQueueInstance() throws Exception {
        ...
        queue.setDataSource(dataSource).setTableName(tableQueue).setEphemeralDisabled(false).init();
        queue.flush();
