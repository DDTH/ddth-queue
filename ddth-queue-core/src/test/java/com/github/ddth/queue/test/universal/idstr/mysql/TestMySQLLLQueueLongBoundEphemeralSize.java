...
import com.github.ddth.queue.test.universal.idint.mysql.MyLLQueue;
...
/*
 * mvn test -DskipTests=false -Dtest=com.github.ddth.queue.test.universal.idstr.mysql.TestMySQLLLQueueLongBoundEphemeralSize -DenableTestsMySql=true
 */

public class TestMySQLLLQueueLongBoundEphemeralSize extends BaseQueueLongTest<Long> {
    public TestMySQLLLQueueLongBoundEphemeralSize(String testName) {
        super(testName);
    }

    public static Test suite() {
        return new TestSuite(TestMySQLLLQueueLongBoundEphemeralSize.class);
    }
    ...
    protected IQueue<Long, byte[]> initQueueInstance() throws Exception {
        ...
        queue.setDataSource(dataSource).setTableName(tableQueue).setEphemeralDisabled(false).setEphemeralMaxSize(16)
                .init();
        queue.flush();
